...
func NewHub(ctx context.Context, cancel context.CancelFunc, db *bolt.DB, httpc *http.Client, hcfg *HubConfig, limiter *RateLimiter) (*Hub, error) {
	...
	err = db.Update(func(tx *bolt.Tx) error {
		...
		if txFeeReserveB == nil {
			...
			h.txFeeReserve = bitumutil.Amount(0)
			tbytes := make([]byte, 4)
		...
		if txFeeReserveB != nil {
			...
			h.txFeeReserve = bitumutil.Amount(feeReserve)
		}
	...
	ntfnHandlers := &rpcclient.NotificationHandlers{
		...
		// TODO: Switch to OnWork notifications when bitumd PR #1410 is merged.
		// OnWork: func(headerE string, target string) {
		// 	h.processWork(headerE, target)
		// },
	}

	// Establish RPC connection with bitumd.
	rpcc, err := rpcclient.New(hcfg.BitumdRPCCfg, ntfnHandlers)
	if err != nil {
		return nil, fmt.Errorf("rpc error (bitumd): %v", err)
	}
	...
	// TODO: Subscribe for OnWork notifications when bitumd PR #1410 is merged.
	// if err := h.rpcc.NotifyWork(); err != nil {
	// 	h.rpccMtx.Lock()
	// 	h.rpcc.Shutdown()
	// 	h.rpccMtx.Unlock()
	// 	return nil, fmt.Errorf("notify work rpc error (bitumd): %v", err)
	// }
	...
	if err := h.rpcc.NotifyBlocks(); err != nil {
		...
		h.rpcc.Shutdown()
		h.rpccMtx.Unlock()
		return nil, fmt.Errorf("notify blocks rpc error (bitumd): %v", err)
	}

	log.Infof("RPC connection established with bitumd.")
	...
	if !h.cfg.SoloPool {
		creds, err := credentials.NewClientTLSFromFile(hcfg.WalletRPCCertFile,
			"localhost")
		if err != nil {
			return nil, fmt.Errorf("grpc tls error (bitumwallet): %v", err)
		}

		h.gConn, err = grpc.Dial(hcfg.WalletGRPCHost,
			grpc.WithTransportCredentials(creds))
		if err != nil {
			return nil, fmt.Errorf("grpc dial error (bitumwallet): %v", err)
		}
	...
	for {
		...
		case headerB := <-h.connCh:
			...
			if !h.cfg.SoloPool {
				...
				coinbase :=
					bitumutil.Amount(block.Transactions[0].TxOut[2].Value)
...
func (h *Hub) ProcessPayments(height uint32) error {
	...
	// details.
	pmts := make(map[bitumutil.Address]bitumutil.Amount, len(details))
