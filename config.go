...
const (
	defaultConfigFilename  = "bitumpool.conf"
	defaultDataDirname     = "data"
	defaultLogLevel        = "debug"
	defaultLogDirname      = "log"
	defaultLogFilename     = "bitumpool.log"
	defaultDBFilename      = "bitumpool.kv"
	defaultTLSCertFilename = "bitumpool.cert"
	defaultTLSKeyFilename  = "bitumpool.key"
	defaultRPCCertFilename = "rpc.cert"
	defaultRPCUser         = "bitump"
	defaultRPCPass         = "bitumppass"
	defaultBitumdRPCHost     = "127.0.0.1:9209"
	defaultWalletGRPCHost  = "127.0.0.1:9211"
	defaultPoolFeeAddr     = ""
...
var (
	...
	bitumpoolHomeDir       = bitumutil.AppDataDir("bitumpool", false)
	bitumwalletHomeDir     = bitumutil.AppDataDir("bitumwallet", false)
	bitumdHomeDir          = bitumutil.AppDataDir("bitumd", false)
	defaultConfigFile    = filepath.Join(bitumpoolHomeDir, defaultConfigFilename)
	defaultDataDir       = filepath.Join(bitumpoolHomeDir, defaultDataDirname)
	defaultDBFile        = filepath.Join(defaultDataDir, defaultDBFilename)
	bitumdRPCCertFile      = filepath.Join(bitumdHomeDir, defaultRPCCertFilename)
	walletRPCCertFile    = filepath.Join(bitumwalletHomeDir, defaultRPCCertFilename)
	defaultLogDir        = filepath.Join(bitumpoolHomeDir, defaultLogDirname)
	defaultTLSCertFile   = filepath.Join(bitumpoolHomeDir, defaultTLSCertFilename)
	defaultTLSKeyFile    = filepath.Join(bitumpoolHomeDir, defaultTLSKeyFilename)
)
...
type config struct {
	...
	ActiveNet       string   `long:"activenet" ini-name:"activenet" description:"The active network being mined on. {testnet, mainnet, simnet}"`
	...
	BitumdRPCHost     string   `long:"bitumdrpchost" ini-name:"bitumdrpchost" description:"The ip:port to establish an RPC connection for bitumd."`
	BitumdRPCCert     string   `long:"bitumdrpccert" ini-name:"bitumdrpccert" description:"The bitumd RPC certificate."`
	...
	MaxTxFeeReserve float64  `long:"maxtxfeereserve" ini-name:"maxtxfeereserve" description:"The maximum amount reserved for transaction fees, in BITUM."`
...
func genCertPair(certFile, keyFile string) error {
	org := "bitumpool autogenerated cert"
...
//
// The above results in bitumpool functioning properly without any config settings
// while still allowing the user to override settings with config files and
// command line options.  Command line options always take precedence.
func loadConfig() (*config, []string, error) {
	// Default config.
	cfg := config{
		HomeDir:         bitumpoolHomeDir,
		...
		BitumdRPCHost:     defaultBitumdRPCHost,
		BitumdRPCCert:     bitumdRPCCertFile,
	...
	// Update the home directory for bitumpool if specified. Since the home
	...
	case chaincfg.TestNetParams.Name:
		cfg.net = &chaincfg.TestNetParams
	...
	if !cfg.SoloPool {
		for _, pAddr := range cfg.PoolFeeAddrs {
			addr, err := bitumutil.DecodeAddress(pAddr)
			if err != nil {
	...
	if !fileExists(defaultTLSCertFile) || !fileExists(defaultTLSKeyFile) {
		...
		if err != nil {
			return nil, nil,
				fmt.Errorf("failed to generate bitumpool's TLS cert/key: %v", err)
		}
	}

	// Load Bitumd RPC Certificate.
	if !fileExists(cfg.BitumdRPCCert) {
		return nil, nil,
			fmt.Errorf("bitumd RPC certificate (%v) not found", cfg.BitumdRPCCert)
	}

	cfg.bitumdRPCCerts, err = ioutil.ReadFile(bitumdRPCCertFile)
	if err != nil {
		return nil, nil, err
	}
