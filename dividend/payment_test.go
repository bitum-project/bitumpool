...
func TestPayPerShare(t *testing.T) {
	...
	amt, err := bitumutil.NewAmount(expectedTotal)
	if err != nil {
		t.Error(err)
	}
...
func CreatePaymentBundle(account string, count uint32, paymentAmount bitumutil.Amount) *PaymentBundle {
...
func TestEqualPaymentDetailsGeneration(t *testing.T) {
	...
	count := uint32(3)
	pmtAmt, _ := bitumutil.NewAmount(10.5)
	...
	zeroAmt := bitumutil.Amount(0)
	txFeeReserve := bitumutil.Amount(0)

	details, totalAmt, err := GeneratePaymentDetails(db,
		[]bitumutil.Address{poolFeeAddrs}, bundles, zeroAmt, &txFeeReserve)
	if err != nil {
		t.Error(err)
	}
...
func TestUnequalPaymentDetailsGeneration(t *testing.T) {
	...
	count := uint32(2)
	accXAmt, _ := bitumutil.NewAmount(9.5)
	accYAmt, _ := bitumutil.NewAmount(4)
...
func TestPaymentsMaturity(t *testing.T) {
	...
	height := uint32(20)
	feePercent := 0.1
	amt, err := bitumutil.NewAmount(100.25)
	if err != nil {
		t.Error(err)
	}
	...
	minPayment, err := bitumutil.NewAmount(0.2)
	if err != nil {
		t.Error(err)
	}
